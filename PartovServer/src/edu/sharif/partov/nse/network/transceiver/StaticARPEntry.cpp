...
void StaticARPEntry::update (
    const edu::sharif::partov::nse::network::address::MACAddress &) {
  throw new edu::sharif::partov::exception::Exception ("This ARP entry is immutable");
}

bool StaticARPEntry::isTimedOut () const {
  return false;
}

const edu::sharif::partov::nse::network::address::MACAddress &
StaticARPEntry::getMACAddress () const {
  return mac;
}
