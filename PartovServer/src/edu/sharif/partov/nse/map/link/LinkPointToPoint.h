...
#ifndef LINK_POINT_TO_POINT_H_
#define LINK_POINT_TO_POINT_H_
...
/**
 * Name:              LinkPointToPoint
 * Parent:            QObject
 * Description:       Represents a link in the virtual network running 802.3.
 *                    Exactly one client must send packets over this link and
 *                    exactly one other server (this may be relaxed) must receive
 *                    sent packets.
 *
 ...
 * Tags:              None;
 */
class LinkPointToPoint : public QObject {
...
public:
  LinkPointToPoint (const QString &linkName, Link *parent);
  virtual ~LinkPointToPoint ();
